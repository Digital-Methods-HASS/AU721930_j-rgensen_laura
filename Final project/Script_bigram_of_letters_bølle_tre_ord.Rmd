...
```{r inddeler i bigram}
all_texts %>% 
  unnest_tokens(bigram, text , token = "ngrams", n=3) -> brevebølle_bigrams
...
brevebølle_bigrams %>% 
  separate(bigram, c("word1", "word2", "word3"), sep = " ") ->brevebølle_bigrams_separated
...
brevebølle_bigrams_separated %>% 
  filter(!word1 %in% stopord) %>%
  filter(!word2 %in% stopord) %>% 
  filter(!word3 %in% stopord)-> brevebølle_bigrams_filtered


brevebølle_bigrams_filtered
...
```{r}
brevebølle_bigrams_filtered %>% 
  filter(str_detect(word2, "\\bbef+al+[a-zæø]*|\\bgøy[a-zæø]*|\\bgud[a-zæø]*")) %>% 
  count(word1, word2, word3, sort = TRUE) -> brevebølle_test_bigrams_counts


brevebølle_test_bigrams_counts
...
```{r, message=FALSE}
library(igraph)
bigram_graph <- brevebølle_test_bigrams_counts %>%
