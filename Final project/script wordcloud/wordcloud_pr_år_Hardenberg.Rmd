...
list.files(path = "data/breve_hardenberg", pattern = "pdf")
```
```{r}
#Følger guiden fra software carpentry 
list.files(path = "data/breve_hardenberg", pattern = "breve_Hardenberg,Anna", full.names = TRUE)
```
```{r}
#Følger guiden fra software carpentry 
filenames <- list.files(path = "data/breve_hardenberg",  
                        pattern = "breve_Hardenberg,Anna-[0-9]{2}.pdf",
                        full.names = TRUE)
for (f in filenames) {
  print(f)
  analyze(f)
...
breve<-all_words_tf_idf %>%
  ...
  scale_color_gradient(low = "darkgoldenrod2", high = "darkgoldenrod4") +
  labs(
    title = "Anna Hardenberg letters: most important words per year",
    subtitle = "Importance determined by term frequency (tf) - inversed document frequency (idf)",
...
```{r}
ggsave(plot = breve, 
       here("figures","Anna_Hardenberg_wordcloud.png"),width = 10, height = 6, dpi = 300)
```
